...
export default function AppointmentForm({
  ...
  const getSpecialtyUrl = (searchTerm: string) => {
    return `${config.apiUrl}/Specialty${
  ...
  const getAppointmentTime = () => {
    return doctor?.shifts
      ...
      .map((shift) => {
        return { name: `${shift.startTime}-${shift.finishTime}` };
      });
  };
    ...
    <Wrapper>
      ...
      <main>
        <DropdownInput
          ...
          handlePick={(value: string) => setSpecialty(value)}
          transformFunction={(list: Specialty[]) => {
        ...
        />
        <DropdownInput
          getListUrl={getDoctorUrl}
          icon={SearchIcon}
          type="Bác sĩ"
          label="Doctor"
          labelText="Bác sĩ"
          placeholder="Chọn bác sĩ"
          transformFunction={(list: Doctor[]) => {
            return list.map((item) => {
              return {
                name: item.name,
                img: item.imageUrl,
          ...
          <Controller
            ...
            render={({
              ...
              <Textarea
                {...field}
                name="reason"
                labelText="Lý do đặt lịch"
                placeholder="Vd: Khám bệnh định kỳ"
              />
            )}
          />
          <Controller
        ...
        <section className="flex-container">
          <div style={{ flexBasis: "55%" }}>
            ...
            {showCalendar && (
              <DatePicker
                handler={updateDate}
                weekdays={doctor?.shifts.map((item) => item.weekday)}
                pastDisabled={true}
              />
            )}
          </div>
          <div style={{ flexBasis: "45%" }}>
            <DropdownInput
              transformFunction={(list: { name: string }[]) => {
                return list.map((item) => {
                  return { name: item.name, value: item.name };
                });
              }}
              items={getAppointmentTime()}
              icon={ClockIcon}
              type="Thời gian khám"
              ...
              disabled={true}
              handlePick={(value: any) => setValue("appointmentTime", value)}
            />
          </div>
        </section>
      </main>
