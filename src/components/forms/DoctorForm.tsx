...
export default function DoctorForm({
  ...
  const [specialties, setSpecialties] = useState<string[]>(
    doctor && doctor.specialties ? doctor.specialties : []
  );
  ...
  const onSubmit = async (data: DoctorInfo) => {
    try {
      ...
      if (doctor) {
        await axios.put(
          `${config.apiUrl}/Doctor/${doctor.doctorId}`,
          formData,
          {
            headers: {
