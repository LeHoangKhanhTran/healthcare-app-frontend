...
export default function DropdownInput<T>({
  ...
  const [shouldDisable, setShouldDisable] = useState<boolean | undefined>(
    disabled
  );
  useEffect(() => {
    if (!items) {
  ...
  const handler = (value: any) => {
    handlePick(
      (value as Item).value ? (value as Item).value : (value as Item).name,
      list
    );
    setValue(value);
  };
  ...
  return (
    <Wrapper className="dropdown-input" ref={ref}>
      <Input
        icon={icon}
        labelText={labelText}
        placeholder={type !== "Bác sĩ" || !pickedValue ? placeholder : ""}
        disabled={shouldDisable}
        value={getInputValue(pickedValue)}
        onChange={(e) => {
      ...
      >
        {type === "Bác sĩ" && pickedValue && (
          <ItemCard name={pickedValue.name} img={pickedValue.img} />
        )}
        <img
