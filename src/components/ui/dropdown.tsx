    ...
    <Wrapper $top={top} $width={width} className="dropdown">
      ...
      {itemList &&
        itemList.map((item, i) => {
          let picked =
            pickedOption &&
            JSON.stringify(item.value ? item.value : item.name) ===
            JSON.stringify(pickedOption.value ? pickedOption.value : pickedOption);
          return (
            <DropdownItem key={i} item={item} onClick={pickHandler} picked={picked}/>
          );
        })}
    </Wrapper>
  );
