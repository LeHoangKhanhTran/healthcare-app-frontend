...
const OtpSlot = styled.input<{ active: boolean }>`
  ...
  border-radius: 8px;
  border: 2px solid
    ${(props) => (props.active ? "var(--green)" : "var(--grey-border-color)")};
  font-size: 2rem;
  ...
  const handleKeyDown = (
    ...
    if (e.key === "Backspace") {
      ...
      if (index === currentIndex) {
        updatedOtp =
          otp.substring(0, currentIndex - 1) +
          " " +
          " " +
          otp.substring(currentIndex + 1, otp.length);
        ...
        updatedOtp =
          otp.substring(0, index) + " " + otp.substring(index + 1, otp.length);
      }
      ...
      if (!isNaN(input) && (currentIndex < 6 || index < currentIndex)) {
        ...
        if (index === currentIndex) {
          ...
          updatedOtp =
            ...
            input.toString() +
            otp.substring(index + 1, otp.length);
        }
        setOtp(updatedOtp);
        if (currentIndex < 5) {
          (
            (e.target as HTMLInputElement).nextSibling as HTMLInputElement
          ).focus();
        }
      }
    }
