...
export default function AppointmentAdmin() {
    ...
    const getStats = () => {
        try {
            ...
            const cancelledRequest = axios.get(`${config.apiUrl}/Appointment/2/count`)
            axios.all([scheduledRequest, pendingRequest, cancelledRequest])
            .then(axios.spread((scheduledResponse, pendingResponse, cancelledResponse) => {
                const updatedStats = {
    ...
    const getAppointments = async () => {
        try {
            const response = await axios.get(`${config.apiUrl}/Appointment${statusFilter && order ? `?Status=${statusFilter}&Order=${order}` : 
                statusFilter ? `?Status=${statusFilter}` : order ? `?Order=${order}` :  ''}`);
    ...
    const updateStatus = async (id: string, action: "schedule" | "cancel") => {
        try {
            const response = await axios.patch(`${config.apiUrl}/Appointment/${id}/${action}`);
            if (response.status === 200) {
                getAppointments()
                getStats()
            }
    ...
    return (
        <Container>
            ...
            <section className="stat-group">
                <StatCard type="scheduled" stat={stats["scheduled"]} label="Tổng số lịch đặt khám đã được duyệt" icon={ScheduledIcon}/>
                <StatCard type="pending" stat={stats["pending"]} label="Tổng số lịch đặt khám đang chờ duyệt" icon={PendingIcon}/>
                <StatCard type="cancelled" stat={stats["cancelled"]} label="Tổng số lịch đặt khám đã bị hủy" icon={CancelledIcon}/>
            </section>
                    ...
                    <tbody>
                        {appointments && appointments.map((appointment, i) => {
                            ...
                            <tr className={i % 2 === 0 ? "first-row" : "second-row"}>
                                ...
                                <td className="date">{`${formatDate(appointment.appointmentDate)}-(${appointment.appointmentTime})`}</td>
                                <td>
                                ...
                                <td>
                                    <div className="actions">
                                        <div style={{display: "flex", justifyContent: "center", gap: "20px"}}>
                                            <span className="schedule" onClick={() => updateStatus(appointment.appointmentId, "schedule")}>Duyệt</span>
                                            <span className="cancel" onClick={() => handleCancel(appointment)}>Hủy</span>
                                        </div>
                                    </div>
                                </td>
                            </tr>
            ...
            {modal === "cancel" && appointment && <CancelModal handleClick={() => {updateStatus(appointment.appointmentId, "cancel"); setModal("")}}/>}
        </Container>
    )
}