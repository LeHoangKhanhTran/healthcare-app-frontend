...
const Container = styled.div`
  ...
  padding: 50px;
  gap: 30px;
  background: linear-gradient(var(--input-bg-color), var(--input-bg-color))
      padding-box,
    var(--input-focus-color) border-box;
  ...
  const getProfile = async (profileId: string) => {
    try {
      const response = await axios.get(
        `${config.apiUrl}/PatientProfile/${profileId}`
      );
  ...
  const deleteDocument = async (documentId: string) => {
    try {
      const response = await axios.delete(
        `${config.apiUrl}/PatientProfile/${user?.profileId}/documents?documentId=${documentId}`
      );
      if (response.status === 200) {
      ...
      <>
        <Wrapper>
          ...
          <Container>
            ...
            <section>
              <h2>Các tài liệu</h2>
              {profile.patientDocuments &&
                profile.patientDocuments.map((document) => {
                  return (
                    <div className="document-container">
                      <div className="document">
                        {document.documentFormat === "jpg" ||
                        document.documentFormat === "png" ? (
                          <img
              ...
              {(!profile.patientDocuments ||
                profile.patientDocuments.length === 0) && (
                <div>Chưa có tài liệu nào được đăng tải</div>
              )}
            </section>
            <Button
              onClick={() => {
        ...
        {modal === "document" && (
          <DocumentModal
            profileId={profile.patientProfileId}
            handleClose={() => {
              setModal("");
            }}
            sideTask={getProfile(profile.patientProfileId)}
          />
        )}
      </>
    );
